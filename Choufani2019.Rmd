...
design <- model.matrix(~ sex + groups)
    design
    mxs <- meth[,which( colnames(meth) %in% samplesheet$Basename )]
    fit.reduced <- lmFit(mxs,design)
    fit.reduced <- eBayes(fit.reduced)
    summary(decideTests(fit.reduced))
...
top_NA_vs_invivo_inc <- dm_analysis(samplesheet=samplesheet,
    sex=sex,groups=groups,mx=methss,name="top_NA_vs_invivo_inc",
    myann=myann ,beta= beta)

head(top_NA_vs_invivo_inc)

```


```{r,nat_vs_invivo_excl}
...
top_nat_vs_invivo_exc <- dm_analysis(samplesheet=samplesheet,
    sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_invivo_exc",
    myann=myann, beta=beta_flt)

head(top_nat_vs_invivo_exc)

```

# Nat vs In vitro
```{r,nat_vs_invitro_incl}
...
top_nat_vs_invitro_exc <- dm_analysis(samplesheet=samplesheet,
    sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_invitro_exc",
    myann=myann, beta=beta_flt)

head(top_nat_vs_invitro_exc)
...
head(top_invivo_vs_invitro_exc)
