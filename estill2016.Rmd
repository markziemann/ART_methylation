...
beta=beta_flt
 ...
 design <- model.matrix(~ sex + groups)
  ...
  head(all)
  head(up)
  xx <- data.frame(all,up,fix.empty.names = "TRUE")
  rownames(xx) <- xx[,1]
  xx[,c(1,3)]=NULL
  rownames(xx)[1] = "None"
...
```{r,compartment}
#save.image("estill2016.Rdata")
#load("estill2016.Rdata")

dma <- top_nat_vs_fz$dma
up <- subset(dma,logFC>0 & adj.P.Val<0.05)
dn <- subset(dma,logFC<0 & adj.P.Val<0.05)
...
xx$r1 <- xx$all/sum(xx$all)
xx$r2 <- xx$up/sum(xx$up)
xx$or <- xx$r2 / xx$r1
xx$phyper <- dhyper(x, m, n, k, log = FALSE)
...
comp <- compartment_enrichment(top_nat_vs_fz$dma)
comp
```
...
#unlink("estill_mitch_report.html")
     capture.output(
