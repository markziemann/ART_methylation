...
make_dm_plots(dm = top_nat_vs_fh ,name="top_nat_vs_fh" , mx= Mvalss, groups= group)
...
dim(top_nat_vs_fz)
head(top_nat_vs_fz)
make_dm_plots(dm = top_nat_vs_fz ,name="top_nat_vs_fz" , mx= Mvalss, groups= group)
...
dim(top_nat_vs_iui)
head(top_nat_vs_iui)
make_dm_plots(dm = top_nat_vs_iui ,name="top_nat_vs_iui" , mx= Mvalss, groups= group)
```
...
dim(top_fh_vs_fz)
head(top_fh_vs_fz)
make_dm_plots(dm = top_fh_vs_fz ,name="top_fh_vs_fz" , mx= Mvalss, groups= group)
```
...
dim(top_nat_vs_fx)
head(top_nat_vs_fx)
make_dm_plots(dm = top_nat_vs_fx ,name="top_nat_vs_fx" , mx= Mvalss, groups= group)
```
...
dim(top_iui_vs_fx)
head(top_iui_vs_fx)
make_dm_plots(dm = top_iui_vs_fx ,name="top_iui_vs_fx" , mx= Mvalss, groups= group)
```
...
## Generate heatmaps of the top N probes

Heatmaps give us a visual idea as to how consistent the observed differential methylation patterns are for the top N genes.

## Generate volcano plots

Volcano plots give us an idea about how many differential genes were identified for a particular contrast.
