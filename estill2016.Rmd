...
fit.reduced <- eBayes(fit.reduced)
summary(decideTests(fit.reduced))
top_nat_vs_fh <- topTable(fit.reduced,coef=2,number = Inf)
dim(top_nat_vs_fh)
head(top_nat_vs_fh)
```
...
fit.reduced <- eBayes(fit.reduced)
summary(decideTests(fit.reduced))
top_nat_vs_fz<-topTable(fit.reduced,coef=2)
dim(top_nat_vs_fz)
head(top_nat_vs_fz)
```
...
fit.reduced <- eBayes(fit.reduced)
summary(decideTests(fit.reduced))
top_nat_vs_iui<-topTable(fit.reduced,coef=2)
dim(top_nat_vs_iui)
head(top_nat_vs_iui)
```
...
fit.reduced <- eBayes(fit.reduced)
summary(decideTests(fit.reduced))
top_fh_vs_fz <- topTable(fit.reduced,coef=2)
dim(top_fh_vs_fz)
head(top_fh_vs_fz)
```
...
fit.reduced <- eBayes(fit.reduced)
summary(decideTests(fit.reduced))
top_nat_vs_fx <- topTable(fit.reduced,coef=2)
dim(top_nat_vs_fx)
head(top_nat_vs_fx)
```
...
## To-do
Remaining analysis that is required:
