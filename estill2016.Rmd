...
dm_analysis <- function(samplesheet,sex,groups,mx,name) {
    ...
    dm_up <- rownames(subset(top_nat_vs_fh,"adj.P.Val"<0.05 & "logFC">0))
    dm_dn <- rownames(subset(top_nat_vs_fh,"adj.P.Val"<0.05 & "logFC"<0))
    length(dm_up)
    length(dm_dn)
    make_dm_plots(dm = dm ,name="top_nat_vs_fh" , mx= Mvalss, groups= groups)
    dat <- list("dm"=dm,"dm_up"=dm_up,"dm_dn"=dm_dn)
}
...
top_nat_vs_fh_inc <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval,name="top_nat_vs_fh_inc")
head(top_nat_vs_fh_inc$dm)
...
sex <- factor(samplesheet$sex,levels=c("Male","Female"))
top_nat_vs_fh <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_fh")
head(top_nat_vs_fh$dm)
```
...
sex <- factor(samplesheet$sex,levels=c("Male","Female"))
top_nat_vs_fz <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_fz")
head(top_nat_vs_fz$dm)
```
...
sex <- factor(samplesheet$sex,levels=c("Male","Female"))
top_nat_vs_iui <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_iui")
head(top_nat_vs_iui$dm)
```
...
sex <- factor(samplesheet$sex,levels=c("Male","Female"))
top_fh_vs_fz <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_fh_vs_fz")
head(top_fh_vs_fz$dm)
```
...
top_nat_vs_fx <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_nat_vs_fx")
head(top_nat_vs_fx$dm)
```
...
top_iui_vs_fx <- dm_analysis(samplesheet=samplesheet,sex=sex,groups=groups,mx=Mval_flt,name="top_iui_vs_fx")
head(top_iui_vs_fx$dm)
```
